...
TARGET_COLUMN = 'next_bar_direction' # หรือ 'next_bar_direction' ถ้าใช้การทำนายทิศทาง
...
LSTM_UNITS = 64 # เพิ่มจำนวนหน่วยใน LSTM layer (จาก 50 เป็น 64)
DROPOUT_RATE = 0.3 # เพิ่ม Dropout rate (จาก 0.2 เป็น 0.3)
...
def _load_and_create_features_from_csv(file_path):
    ...
    try:
        ...
        # Define the target variable (assuming 0 for Sell, 1 for Buy)
        # Shift close price to predict next bar's close
        df['next_close'] = df['Close'].shift(-1)
        # If next_close is higher than current close, then it's a 'Buy' (1), else 'Sell' (0)
        # We need to handle the last row which will have NaN for next_close
...
if __name__ == '__main__':
    ...
    # This requires the original non-one-hot encoded 'y' array for computing class weights
    # Note: `y` here is the original numerical labels (0, 1) before one-hot encoding.
    ...
    try:
        ...
        print("\nClassification Report:")
        print(classification_report(y_true, y_pred, target_names=[f'Class {i}' for i in range(num_classes)])) # Adjust target names as needed
    ...
    try:
        ...
        # Calculate returns as simple +1 for correct prediction, -1 for incorrect prediction
        backtest_df['strategy_return'] = np.where(predicted_classes == y_backtest, 1, -1)
        ...
        # This means simply holding from the start of the backtest period
        # Need to ensure the index for backtest_df starts correctly relative to original prices
        # The original combined_df needs to be accessible with its original index
        original_combined_df_for_bh = load_all_data(DATA_PATH) # Reload original data for accurate B&H
        # Need to ensure this reloaded df matches combined_df's index after cleaning/aligning
        
        # A simpler way for B&H without reloading:
        # Assuming backtest_df's 'Close' column reflects actual close prices for the backtesting period
