...
# Import necessary sklearn components for training part (if this script is also used for training)
...
# --- Focal Loss for imbalanced datasets (if needed for training) ---
...
def fetch_and_prepare_data(symbol, sequence_length, features_list):
    ...
    # A generous buffer (e.g., 100 bars) for indicator calculations and dropna
    safety_bars = 100 # Increased safety_bars
    ...
    # Added error message for failed data fetching
    if h1_raw_data is None or len(h1_raw_data) == 0:
        print(f"MT5 Error (H1): {mt5.last_error()}")
    ...
    if m15_raw_data is None or len(m15_raw_data) == 0:
        print(f"MT5 Error (M15): {mt5.last_error()}")
    ...
    if h4_raw_data is None or len(h4_raw_data) == 0:
        print(f"MT5 Error (H4): {mt5.last_error()}")
    ...
    print(f"DEBUG: df_h1_features shape before merge: {df_h1_features.shape}")
    print(f"DEBUG: df_m15_features shape before merge: {df_m15_features.shape}")
    print(f"DEBUG: df_h4_features shape before merge: {df_h4_features.shape}")
    ...
    print(f"DEBUG: df_combined shape after merge_asof: {df_combined.shape}")
    df_combined.dropna(inplace=True)
    print(f"DEBUG: df_combined shape after dropna: {df_combined.shape}")
    ...
    if len(latest_sequence_df) < sequence_length:
        raise ValueError(f"Not enough data to form a complete sequence of length {sequence_length} after alignment and feature selection. Final DF size: {len(latest_sequence_df)}")
...
def main_file_monitor():
    ...
    if not mt5.initialize(): # This will attempt to connect or check if already connected
        print("❌ Failed to initialize MetaTrader5 on startup. Please check your MT5 installation and path, and ensure MT5 terminal is running.")
        sys.exit() # Critical error, exit if MT5 cannot be initialized
    print("✅ MetaTrader5 initialized successfully (at main_file_monitor start).") # Confirm connection here
        ...
        try:
            ...
            if os.path.exists(input_file_path):
                ...
                if current_modified_time > LAST_INPUT_FILE_MOD_TIME:
                    ...
                    # Changed encoding to 'utf-8' and ensure_ascii=False for better compatibility with MQL5 UTF-8 reading
